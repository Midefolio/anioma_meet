...
import { deleteCustomers, updateCustomers } from "../../data/apis";

const CustomerDetails = ({getCustomers, i, setSd}:any) => {
  const [isUploading, setIsUploading] = useState<boolean>(false);
  const [customer, setCustomer] = useState<any>(i);
  ...
  const addCustomer = async() => {
    ...
    isSending(true, 'updating...')
    const cb =()=> {isSending(false, "")}
    const res = await makeRequest("POST", updateCustomers, customer, cb, token)
    if(res) {
      await getCustomers();
      setSd(false)
    }
  ...
  return (
    <>
      <div className="my-modal" onClick={(e)=> {e.stopPropagation(); setSd(false)}}>
