/* eslint-disable @typescript-eslint/no-unused-vars */
/* eslint-disable @typescript-eslint/no-explicit-any */

import { useState } from "react";
import { AiOutlineClose, AiOutlineEye } from "react-icons/ai";
import { CiSearch } from "react-icons/ci";
import NewInvoice from "./new_invoice";
import InvoiceDetails from "./invoice_details";
import useUserAuthContext from "../../hook/userUserAuthContext";
import CustomSkeleton from "../../component/skeleton";
import { formatNumber } from "../../utils/useutils";

const InvoiceTable = () => {
  const {Invoices, fi, getInvoices, setFilteredInvoices} = useUserAuthContext(); // Changed to an array of Invoices
  const [searchQuery, setSearchQuery] = useState<string>(""); // New state for search input
  const [newM, setNew] = useState(false)
  const [sd, setSd] = useState<any>()

  // Function to handle search input change
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    const query = event.target.value.toLowerCase();
    setSearchQuery(query);

    if (query === "") {
      setFilteredInvoices(Invoices); // Reset to default when search is cleared
    } else {
      const filtered = Invoices.filter((invoice: { customer: string; }) =>
        invoice?.customer?.toLowerCase().includes(query)
      );
      setFilteredInvoices(filtered);
    }
  };

  // Clear search input and reset table
  const clearSearch = () => {
    setSearchQuery("");
    setFilteredInvoices(Invoices); // Reset to original Invoices
  };


//  const deleteMeet = async (_id:string) => {
//   const confirm = window.confirm("are you sure you want to delete this Invoices ?");
//   if(!confirm) {return}
//   isSending(true, "deleting...");
//   const cb = () => {
//     isSending(false, "");
//   };
//   const res = await makeRequest("DELETE", deleteInvoices, {_id}, cb, token);
//   if (res) {
//     await getInvoices()
//     isSending(false, "");
//   }
// };

  return (
    <>
      {newM && <NewInvoice getInvoices={getInvoices} setNew={setNew} />}
      
      {fi?.map((i: any, _index: number) => (<>
       {sd === i?._id && <InvoiceDetails getInvoices={getInvoices} i={i} setSd={setSd} />}
      </>))}

      <div className="gap-20">
        <div className="gap-elements centered-align fla-border-1 my-col-3 rad-30 h-20 bg-color-code-3 my-b-shadow">
          <input
            type="text"
            placeholder="Search table"
            className="input my-col-9 px9 InterLight faded-sol"
            value={searchQuery} // Controlled input
            onChange={handleSearch} // Call the search handler
          />
          {/* Conditionally render the close icon based on the searchQuery value */}
          {searchQuery && (
            <span className="my-col-1 centered-align" onClick={clearSearch}>
              <AiOutlineClose className="px9 color-code-1 pd-2 c-pointer" />
            </span>
          )}
          <span className="my-col-2 centered-align ">
            <CiSearch className="pd-5 px13" />
          </span>
        </div>
        <span
          className="pd-10 bg-color-code-1 InterSemiBold px9 rad-10 c-pointer"
          onClick={() => {
            setNew(true)
          }}
        >
          New Invoices <i className="fas fa-plus mg-5"></i>
        </span>
      </div>

      <div className="my-mother down-2">
        <div className="table-row bg-color-code-1 centered-align rad-10 my-b-shadow gap-20 pd-10">
          <input type="checkbox" />
          <span className="px9 alice InterSemiBold w-30">SN</span>
          <span className="px9 alice w-250 InterSemiBold">Customer</span> 
          <span className="px9 alice w-550 InterSemiBold">Narration</span>
          <span className="px9 alice w-150 InterSemiBold">Amount</span>
          <span className="px9 alice w-150 InterSemiBold">Payment type</span>
          <span className="px9 alice w-150 InterSemiBold">Invoice Status</span> 
          <div className="my-col-1">
            <span className="px9 alice InterSemiBold">Action</span>
          </div>
        </div>
        {!fi && <CustomSkeleton/>}
        {fi?.length > 0 ? (
          fi?.map((i: any, index: number) => (
            <div
              key={index}
              onClick={()=> {setSd(i?._id)}}
              className="table-row rad-10 centered-align bg-color-code-3 down-1 my-b-shadow gap-20 pd-10"
            >
              <input type="checkbox" />
              <span className="px9 alice InterSemiBold w-30">{index + 1}</span>
              <div className="w-250">
              <span className="px9 faded-sol InterSemiBold">
                {i?.clientName}
              </span>
              </div>
              <span className="px9 w-550 faded-sol InterSemiBold">
                {i?.narration}
              </span>
              <div className="w-150">
              <span className="px9 faded-sol InterSemiBold">
                {i?.currency}{formatNumber(i?.totalAmount)}
              </span>
              </div>
              <div className="w-150">
              <span className="px9 faded-sol InterSemiBold">
              {i?.payment_type}
              </span>
              </div>
              <div className="w-550">
              <span className="px9 faded-sol InterSemiBold">
                <span className="pd-5 rad-20 fla-border-1 bg-color-code-2 px8 c-pointer">click to view</span>
              </span>
              </div>
              <div className="my-col-1 gap-elements">
                <AiOutlineEye onClick={()=> {setSd(i?._id)}} />
              </div>
            </div>
          ))
        ) : (
         <> {fi?.length < 1 && <div className="centered-align down-2">No Invoices found</div>}</>
        )}
      </div>
    </>
  );
};

export default InvoiceTable;
